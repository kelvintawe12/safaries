    ...
    <ul className="space-y-3">
      {items.map((item, index) => (
        <li key={index} className="flex items-start">
          <Icon className={`h-5 w-5 ${iconColor} mr-2 mt-0.5`} />
          <span>{item}</span>
        </li>
      ))}
    </ul>
  ...
  return (
    <div className="bg-white w-full">
      ...
      <section className="relative h-[60vh] min-h-[400px]">
        <Slideshow
          images={tour.images.map((src) => ({ src }))}
          className="h-full"
        />
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl">
            ...
            <p className="text-gray-700 mb-8">{tour.description.en}</p>
            ...
            <div className="space-y-6 mb-12">
              {tour.itinerary.map((day, index) => (
                <div
                  ...
                  aria-label={`Day ${day.day}: ${day.title?.en ?? 'No title'}`}
                >
                  <h3 className="text-xl font-semibold mb-2">
                    Day {day.day}: {day.title?.en ?? 'No title'}
                  </h3>
                  <p className="text-gray-700">{day.description.en}</p>
                </div>
              ))}
            </div>
            {/* Booking CTA */}
